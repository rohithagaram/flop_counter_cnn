...
def calculator(model, input_res , print_per_layer_stat=True, as_strings=True,
                              input_constructor=None, ost=sys.stdout,
                              verbose=False, ignore_modules=[],
                              custom_modules_hooks={}):

    # few checks before the actual calculation 
    ...
    model_nn = add_flops_counting_methods(model)
    model_nn.eval()

    model_nn.start_flops_count(ost=ost, verbose=verbose,
                                  ignore_list=ignore_modules)

    batch = torch.ones(()).new_empty((1, *input_res))
    _ = model_nn(batch)

    flops_count, params_count = model_nn.compute_average_flops_cost()
    if print_per_layer_stat:
        print_model_with_flops(model_nn, flops_count, params_count, ost=ost)
    model_nn.stop_flops_count()
